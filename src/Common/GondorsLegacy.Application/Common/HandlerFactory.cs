...
using GondorsLegacy.CrossCuttingCorners.ExtensionMethods;
using GondorsLegacy.Application.Decorators;
    ...
    private void AddDecoratedFactories(Type type)
    {
        ...
        for (var i = attributes.Length - 1; i >= 0; i--)
        {
            ...
            var hasDecoratorHandler = (type.HasInterface(typeof(ICommandHandler<>)) &&
                                       Mappings.AttributeToCommandHandler.TryGetValue(attributeType,
                                           out decoratorHandlerType))
                                      || (type.HasInterface(typeof(IQueryHandler<,>)) &&
                                          Mappings.AttributeToQueryHandler.TryGetValue(attributeType,
                                              out decoratorHandlerType));
